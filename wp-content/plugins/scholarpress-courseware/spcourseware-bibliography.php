...
function bib_sort_items($items)
{
    usort($items, 'bib_sort');
    
    return $items;
}

function bib_sort($firstItem, $secondItem)
{
    /* If you have a author_last, will always order by this.
    * If one has an author, and one doesn't, we'll compare
    * the author_last of the one that has an author, to the title of the one that doesn't.
    */
    
    $firstItemAuthor = trim(strtolower($firstItem->author_last));
    $firstItemTitle = trim(strtolower($firstItem->title));
    $secondItemAuthor = trim(strtolower($secondItem->author_last));
    $secondItemTitle = trim(strtolower($secondItem->title));

    if ($firstItemAuthor !='' && $secondItemAuthor != '') {
        if ($firstItemAuthor == $secondItemAuthor) {
            return 0;
        }
        return ($firstItemAuthor > $secondItemAuthor) ? 1 : -1;
    } else {
        ...
        if ($firstItemAuthor == '' && $secondItemAuthor == '') {
            ...
            return ($firstItemTitle > $secondItemTitle) ? 1 : -1;
        } else {
            if ($firstItemAuthor == '') {
                if ($firstItemTitle == $secondItemAuthor) {
                    return 0;
                }
                return ($firstItemTitle > $secondItemAuthor) ? 1 : -1;
            } else {
                // We know the second item Author is an empty string.
                if ($firstItemAuthor == $secondItemTitle) {
                    return 0;
                }
                return ($firstItemAuthor > $secondItemTitle) ? 1 : -1;
            }
        }
    }
}
...
function bibliography_displaylist() 
{
	...
	$biblios = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "bibliography ORDER BY author_last, title ASC");
	
	$biblios = bib_sort_items($biblios);
...
function bib_page($data,$headings='h4')
{
	...
	if ( $start != false )
	{
		...
		if (count($sql_monograph) > 0 )
		{
			...
			$published = $wpdb->get_results("SELECT * FROM " . $table_name . " WHERE type='monograph' ORDER BY author_last, title ASC");
			
			$published = bib_sort_items($published);
		...
		if (count($sql_chapters) > 0 )
		{
			...
			$published = $wpdb->get_results("SELECT * FROM " . $table_name . " WHERE type='volumechapter' ORDER BY author_last, title ASC");
		...
		if (count($sql_article) > 0 )
		{
			...
			$published = $wpdb->get_results("SELECT * FROM " . $table_name . " WHERE type='article' ORDER BY author_last, title ASC");
		...
		if (count($sql_website) > 0)
		{
			...
			$published = $wpdb->get_results("SELECT * FROM " . $table_name . " WHERE type='website' ORDER BY author_last, title ASC");
		...
		if (count($sql_videos) > 0)
		{
			...
			$published = $wpdb->get_results("SELECT * FROM " . $table_name . " WHERE type='video' ORDER BY author_last, title ASC");
		...
		if (count($sql_audios) > 0)
		{
			...
			$published = $wpdb->get_results("SELECT * FROM " . $table_name . " WHERE type='audio' ORDER BY author_last, title ASC");
		...
		if (count($sql_unpublished) > 0 )
		{
			...
			$published = $wpdb->get_results("SELECT * FROM " . $table_name . " WHERE type='unpublished' ORDER BY author_last, title ASC");
